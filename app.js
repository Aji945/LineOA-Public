...
async function loadAllDataUltraFast() {
    ...
    try {
        ...
        const [
            ...
            APIManager.request('getUserActivityStatus') // <--- 新增
        ]);
        ...
        await Promise.all([
            ...
            (() => {
                ...
                if (userActivityStatusResult && userActivityStatusResult.success) {
                    cache.set('userActivityStatus', userActivityStatusResult.data, 120000); // 2分鐘快取
                }
            })()
        ]);
        ...
        // 7. 啟動輕量背景更新（只更新關鍵數據，避免重複）
        setInterval(async () => {
            if (currentUser?.userId) {
                try {
                    const [pointsRes, ticketsRes] = await Promise.all([
                        APIManager.request('getUserPoints', {}, { skipCache: true }),
                        APIManager.request('getLotteryTickets', {}, { skipCache: true })
                    ]);
                    
                    if (pointsRes.success) {
                        UserDataManager.safeUpdate('points', pointsRes.data?.currentPoints || 0, '背景更新');
                    }
                    if (ticketsRes.success) {
                        UserDataManager.safeUpdate('lotteryTickets', ticketsRes.data?.currentTickets || 0, '背景更新');
                    }
                } catch (error) {
...
// 智能背景更新 - 已整合到 loadAllDataUltraFast，此函數已不需要
